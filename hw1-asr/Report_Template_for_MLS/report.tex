...
4 Attention
  ...
  & $QK^\top{\cdot}s \to \mathrm{softmax\_inplace} \to \mathrm{attn}{\cdot}V$; naive writes $O(T_q T_k)$ score matrix to HBM$^\dagger$
  & Memory$^\dagger$ \\
...
$^\dagger$The naive 3-kernel attention is memory-bound due to the $O(T_q T_k)$ score matrix;
